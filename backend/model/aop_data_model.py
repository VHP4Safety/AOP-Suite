...
class AOPNetworkBuilder:
    ...
    def process_sparql_binding(self, binding: Dict[str, Any]):
        ...
        try:
            ...
            if upstream_uri and upstream_uri not in [
                ...
                self._process_key_event(upstream_uri, upstream_title, NodeType.KE, aop_info)
            
            if downstream_uri and downstream_uri not in [
                binding.get("MIE", {}).get("value", ""),
                binding.get("ao", {}).get("value", "")
            ]:
                self._process_key_event(downstream_uri, downstream_title, NodeType.KE, aop_info)
    ...
    def _process_key_event(self, uri: str, title: str, ke_type: NodeType, aop_info: Optional[AOPInfo]):
    ...
    def _process_relationship(self, ker_uri: str, upstream_uri: str, downstream_uri: str):
        """Process a key event relationship"""
        if upstream_uri in self.network.key_events and downstream_uri in self.network.key_events:
...
class CytoscapeNode:
    ...
    def is_gene_node(self) -> bool:
        """Check if this is a gene-related node"""
        return (self.classes in ["ensembl-node", "uniprot-node"] or 
                self.node_type in ["ensembl", "uniprot"] or
                self.id.startswith(("ensembl_", "uniprot_")))
    
    def is_ensembl_node(self) -> bool:
        """Check if this is an Ensembl gene node"""
        return (self.classes == "ensembl-node" or 
                self.node_type == "ensembl" or
                self.id.startswith("ensembl_"))
    
    def is_uniprot_node(self) -> bool:
        """Check if this is a UniProt protein node"""
        return (self.classes == "uniprot-node" or 
                self.node_type == "uniprot" or
                self.id.startswith("uniprot_"))
...
class GeneProteinPair:
    ...
    def to_table_entry(self) -> Dict[str, str]:
        ...
        return {
            ...
            "ensembl_id": self.ensembl_node_id if self.ensembl_node_id != "N/A" else "N/A",
            "uniprot_node_id": self.uniprot_node_id if self.uniprot_node_id != "N/A" else "N/A",
        }
...
class CytoscapeNetworkParser:
    ...
    def get_gene_nodes(self) -> List[CytoscapeNode]:
        ...
        return [node for node in self.nodes if node.is_gene_node()]
...
class GeneTableBuilder:
    ...
    def _create_gene_protein_pairs(self) -> List[GeneProteinPair]:
        ...
        for edge in self.gene_relationships:
            ...
            source_node = self.ensembl_nodes.get(edge.source) or self.uniprot_nodes.get(edge.source)
            target_node = self.ensembl_nodes.get(edge.target) or self.uniprot_nodes.get(edge.target)
    ...
    def _create_pair_from_nodes(self, ensembl_node: CytoscapeNode, uniprot_node: CytoscapeNode) -> Optional[GeneProteinPair]:
        ...
        try:
            ...
            logger.warning(f"Failed to create pair from nodes {ensembl_node.id}, {uniprot_node.id}: {e}")
            return None

    def _get_orphaned_genes(self, existing_pairs: List[GeneProteinPair]) -> List[GeneProteinPair]:
    ...
    def _get_orphaned_proteins(self, existing_pairs: List[GeneProteinPair]) -> List[GeneProteinPair]:
